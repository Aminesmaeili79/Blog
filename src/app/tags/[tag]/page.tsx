  ...
  return (
    <div className="w-full mb-36">
      <div className="flex items-center gap-4 mb-8">
        ...
        <h1 className="poppins text-4xl font-bold">Posts tagged: <span className="text-white underline px-3 py-1 rounded-full">{tag}</span></h1>
      </div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {Array.isArray(post.frontMatter.tags) && post.frontMatter.tags.map((postTag, index) => (
                      <Link
                        key={index}
                        href={`/tags/${postTag}`}
                        className={`px-3 py-1 rounded-full text-sm ${postTag === tag
                          ? 'bg-gray-300 text-gray-950 hover:bg-gray-200'
